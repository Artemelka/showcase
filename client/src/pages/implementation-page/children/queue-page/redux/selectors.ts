...
const queueSelector = (state: QueueStore): QueueState =>
  state[QUEUE_REDUCER_NAME] || INITIAL_STATE;
...
export const queueProgressTasksSelector = createSelector(
  [queueTasksArraySelector],
  (tasks): Array<TaskItem> =>
    tasks.filter(({ status }) => status === 'progress'),
);

export const queueSendingTasksSelector = createSelector(
  [queueTasksArraySelector],
  (tasks): Array<TaskItem> =>
    tasks.filter(({ status }) => status === 'sending'),
);
