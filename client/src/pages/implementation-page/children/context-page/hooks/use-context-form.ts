...
export const useContextForm = (): ReturnValue => {
  const [apiErrors, setApiErrors] = useState<Partial<InputsState>>(INITIAL_ERRORS_STATE);
  const [, dispatch] = useContextState();

  const { isLoading, sendRequest } = useRequest<InputsState, SignUpResponse, SignUpResponse>({
    onError: (error) => setApiErrors({ userName: error.additionalErrors[0].userName }),
    onSuccess: (response) => {
      dispatch(signUpSuccess(response.data.name));
      clearForm();
    },
    request: ({ userName, password}) => API.SIGN_UP.SIGN({ userName, password }),
  });
