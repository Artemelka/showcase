...
const STATUSES: Array<TodoItemStatus> = ['todo', 'ready', 'in-progress', 'testing', 'reopen', 'done', 'resolved'];

function getRandomStatus() {
  return STATUSES[getRandomNumber(7)];
}

function generateItems(category: string, quantity: number, parentId?: string): Array<TodoItem> {
...
function generateChildrenCategory(parentList: Array<TodoItem>, category: string, quantity: number): Array<TodoItem> {
  return parentList.reduce((acc: Array<TodoItem>, item) =>
