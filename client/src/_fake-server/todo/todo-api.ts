...
export class TodoApi {
  ...
  constructor() {
    ...
    this.getTodoItem = (itemId: string) => {
      const item = state.todoList.filter((item) => item.id === itemId);
    ...
    this.getChildren = ({ parentId, limit, offset, statuses }: GetChildrenParams) => {
      const childrenList = state.todoList.filter((item) => item.parentId === parentId);
    ...
    this.getCategory = ({ category, limit, offset, statuses }: GetCategoryParams) => {
